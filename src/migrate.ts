...
/**
 * Creates the first admin from env vars, or synchronizes
 * an existing admin with the configured phone/password.
 */
export async function bootstrapAdmin(
  ...
  if (!phone) {
    throw new Error("ADMIN_BOOTSTRAP_PHONE is not a valid phone number.");
  }
  ...
  // Generate the new password hash from the password currently
  // configured in Render environment variables.
  const hash = await argon2.hash(password, ARGON2_OPTIONS);

  const existing = await pool.query(
    "SELECT 1 FROM admins WHERE phone_normalized = $1",
    [phone],
  );

  if (existing.rowCount && existing.rowCount > 0) {
    await pool.query(
      "UPDATE admins SET password_hash = $2, updated_at = NOW() WHERE phone_normalized = $1",
      [phone, hash],
    );
